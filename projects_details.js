...
if (actions) {
  ...
  // ===== Media helper (supports image/youtube/fileVideo) =====
  function getYouTubeId(obj) {
    // supports: { id }, { videoId }
    return obj?.id || obj?.videoId || "";
  }

  function renderMediaItem(item) {
    // 1) YouTube item
    if (item?.type === "youtube") {
      const yt = getYouTubeId(item);
      if (!yt) return document.createElement("div");

      const wrap = document.createElement("div");
      wrap.className = "pd-media-slot";
    ...
    if (item?.type === "fileVideo") {
      ...
      (Array.isArray(item.sources) ? item.sources : []).forEach((s) => {
        if (!s?.src) return;
    ...
    // 3) Default: image item (expects item.src)
    ...
    if (item?.caption) {
      ...
      cap.innerHTML = renderWithHighlight(item.caption);
      fig.appendChild(cap);
    }
...
if (section.type === "text") {
  ...
  p.className = "pd-text";
  p.innerHTML = renderWithHighlight(section.content || "");
  ...
  return container;
    ...
    if (section.type === "list") {
      ...
      (Array.isArray(section.items) ? section.items : []).forEach((item) => {
        ...
        li.innerHTML = renderWithHighlight(item);
        ul.appendChild(li);
      });
    ...
    if (section.type === "image") {
      ...
      if (section.caption) {
        const cap = document.createElement("figcaption");
        cap.className = "pd-figure-cap";
        cap.innerHTML = renderWithHighlight(section.caption);
    ...
    // YOUTUBE (section-level)
    if (section.type === "youtube") {
      const yt = getYouTubeId(section);
      const slot = document.createElement("div");
      slot.className = "pd-media-slot";

      if (!yt) return container;
    ...
    // FILE VIDEO (section-level)
    if (section.type === "fileVideo") {
      container.appendChild(renderMediaItem(section));
      return container;
    }

    // GALLERY (number buttons) — NOW supports image + youtube + fileVideo items
